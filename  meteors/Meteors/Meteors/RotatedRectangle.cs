...
/// <summary>
/// A rectangle that takes rotation into account when determining collisions.
/// Shamelessly stolen from George W. Clingerman (http://www.xnadevelopment.com/tutorials/rotatedrectanglecollisions/rotatedrectanglecollisions.shtml)
/// </summary>
public class RotatedRectangle
{
    ...
    public RotatedRectangle Scale(float scale, int xOffset, int yOffset)
    {
        int scaledX = (int)(CollisionRectangle.X + CollisionRectangle.Width * (1 - scale) / 2) + xOffset;
        int scaledY = (int)(CollisionRectangle.Y + CollisionRectangle.Height * (1 - scale) / 2) + yOffset;
        int scaledWidth = (int)(CollisionRectangle.Width * scale);
        int scaledHeight = (int)(CollisionRectangle.Height * scale);

        Rectangle scaledCollisionRect = new Rectangle(scaledX, scaledY, scaledWidth, scaledHeight);
        return new RotatedRectangle(scaledCollisionRect, Rotation);
    }

    public RotatedRectangle Scale(float scale)
    {
